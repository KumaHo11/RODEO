...
export default function CalculadoraPage() {
  ...
  const loadFieldData = useCallback(async () => {
    ...
    try {
      ...
      // Falla silenciosa — se usan defaults
    } finally {
  ...
  const set = <K extends keyof CalculatorInput>(key: K, val: CalculatorInput[K]) => {
    ...
    // Al editar, quita el tag "dato real"
  ...
  return (
    <div className="flex flex-col gap-6 pb-10">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[10px] text-gray-400 mr-1">Escenario activo:</span>
        ...
        <button
          ...
          className={clsx(
            'ml-auto px-4 py-1.5 rounded-lg text-xs font-bold transition-all border-2',
            showComparison
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 items-start">
        ...
        <div className="space-y-5 order-2 lg:order-1">
          ...
          <Section title="Rodeo">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <SliderControl
                ...
                tooltip="Total de animales del rodeo. Junto con el peso y categoría determina los Equivalentes Vaca totales."
          ...
          <Section title="Disponibilidad forrajera">
            ...
            <SliderControl
              ...
              tooltip="Índice de Diferencia de Vegetación Normalizada (satélite). 0 = suelo desnudo, 1 = vegetación densa. Afecta el runoff y el multiplicador de crecimiento."
          ...
          <Section title="Variables climáticas">
            ...
            <div className="space-y-2">
              <label className="text-xs text-gray-700">Índice de sequía</label>
        ...
        <div className="space-y-4 order-1 lg:order-2 lg:sticky lg:top-4">
          ...
          <div className="bg-white border border-gray-100 rounded-xl p-5 flex flex-col items-center gap-2">
            ...
            <div className="w-full mt-2 space-y-1 text-[10px] text-gray-400 border-t border-gray-50 pt-3">
              <p>Estación: <span className="text-gray-600">{result.season.toLowerCase()}</span></p>
              <p>Multiplicador climático: <span className="text-gray-600">×{result.climateMultiplier}</span></p>
              <p>ET estimada: <span className="text-gray-600">{result.et} mm/día</span></p>
              <p>Balance hídrico 7 d: <span className={clsx(
                result.balanceHidricoMm >= 0 ? 'text-green-600' : 'text-red-500'
              )}>{result.balanceHidricoMm >= 0 ? '+' : ''}{result.balanceHidricoMm} mm</span></p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <MetricCard
              ...
              unit="kg MS/cab"
              tooltip="Materia seca consumida por cabeza por día, corregida por el peso y categoría real del rodeo."
            />
            <MetricCard
          ...
          <div className="bg-white border border-gray-100 rounded-xl px-4 py-3.5 space-y-1.5">
            <p className="text-[10px] text-gray-500 uppercase tracking-wide">Equivalentes vaca totales</p>
            <p className="text-2xl text-gray-800 tabular-nums">{result.totalEv.toFixed(1)}<span className="text-sm text-gray-400 ml-1.5">EV</span></p>
            <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 pt-1 border-t border-gray-50">
              <p className="text-[10px] text-gray-400">Crecimiento: <span className="text-gray-600">{result.crecimientoDiarioTotal.toFixed(0)} kg MS/día</span></p>
              <p className="text-[10px] text-gray-400">Precip. efectiva: <span className="text-gray-600">{result.precipEfectivaMm.toFixed(1)} mm</span></p>
              <p className="text-[10px] text-gray-400">ET estimada: <span className="text-gray-600">{result.et} mm/día</span></p>
              <p className="text-[10px] text-gray-400">C_adj: <span className="text-gray-600">×{result.climateMultiplier}</span></p>
            </div>
          </div>
        </div>
      </div>
      ...
      {showComparison && (
        <div ref={comparisonRef} className="bg-white border border-gray-100 rounded-xl overflow-hidden mt-6 scroll-mt-6">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="text-sm font-bold text-gray-800">Comparación de escenarios</h2>
            <p className="text-xs text-gray-500 mt-1">
              Sequía vs condiciones óptimas, calculados sobre la base actual del campo.
