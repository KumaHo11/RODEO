...
export async function GET(req: NextRequest) {
  ...
  const yearFromStr = searchParams.get('year_from') || '2019'
  ...
  // ── 2. Build list of all months to process ────────────────────────────────
  ...
  for (let i = 0; i < pending.length; i += BATCH_SIZE) {
    ...
    await Promise.all(batch.map(async (dateStr) => {
      try {
        ...
        const { indices, captureDate, source, sceneId, cloudCover } = result

        if (!indices.length) { skipped++; return }

        // Insert each index for this month
        await Promise.all(indices.map(idx =>
          ...
          `, [
            ...
            captureDate, source, sceneId, cloudCover,
            idx.confidence || 'HIGH',
            JSON.stringify({ scene_id: sceneId, cloud_cover: cloudCover }),
          ])
        ))
        inserted += indices.length
      } catch (err: any) {
...
// ─────────────────────────────────────────────────────────────────────────────
// Core: compute all indices for a single month directly (no HTTP round-trip)
// ─────────────────────────────────────────────────────────────────────────────
...
async function computeMonthIndices(
  ...
  const fallback = (reason: string): MonthResult => ({
  ...
  try {
    ...
    if (!stacRes.ok) return fallback('stac_error')
    stacData = await stacRes.json()
  } catch {
    return fallback('stac_timeout')
  }

  const items = stacData?.features
  if (!items?.length) return fallback('no_scenes')
  ...
  if (!redUrl || !nirUrl) return fallback('missing_bands')

  // ─ STEP 2: Fetch band stats from TiTiler in parallel ─────────────────────
  ...
  if (!redStats || !nirStats) return fallback('titiler_failed')
...
// ─── Deterministic estimated indices (all 10) ─────────────────────────────────
